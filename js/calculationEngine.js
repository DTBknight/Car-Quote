...
export class CalculationEngine {
  ...
  calculatePurchaseCost(components) {
    const { invoicePrice, purchaseTax = 0, serviceFee = 0, domesticShipping, portCharges = 0, compulsoryInsurance, otherExpenses, qualificationFee = 0, agencyFee = 0, taxRefundFee = 0, taxRefund } = components;
    return invoicePrice + purchaseTax + serviceFee + domesticShipping + portCharges + compulsoryInsurance + otherExpenses + qualificationFee + agencyFee + taxRefundFee - taxRefund;
  }
  ...
  calculateNewCarServiceFee() {
    ...
    if (cached !== null) {
      Utils.setElementText('serviceFee', Utils.formatCurrencyInteger(Math.round(cached)));
    ...
    Utils.setElementText('serviceFee', Utils.formatCurrencyInteger(roundedFee));
