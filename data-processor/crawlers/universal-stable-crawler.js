  ...
  async crawlSingleCarStable(browser, carId, carName) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // 使用奥迪成功的采集方法，增加Protocol error处理
        const carData = await this.dataCollector.handleProtocolTimeout(
          () => this.dataCollector.collectSingleCarData(browser, carId, this.brandName),
          `车型${carName}数据采集`
        );
  ...
  async safeCloseBrowser(browser) {
    ...
    try {
      ...
      if (browser.isConnected()) {
        await this.browserManager.closeBrowser(browser);
        logger.dataCollection('浏览器已安全关闭');
      } else {
        logger.warning('浏览器连接已断开，跳过关闭操作');
      }
    } catch (error) {
      // 处理Protocol error: Connection closed等错误
      if (error.message.includes('Protocol error') || 
          error.message.includes('Connection closed') ||
          error.message.includes('Target closed')) {
        logger.warning(`浏览器关闭时连接已断开: ${error.message.substring(0, 50)}`);
      } else {
        logger.warning(`浏览器关闭失败: ${error.message.substring(0, 50)}`);
      }
    }
  }
  ...
  async crawlBrand() {
    ...
    try {
      ...
      if (checkpoint) {
        ...
        // 显示初始采集进度
        logger.title(`${this.brandName} 品牌车型采集`);
        logger.carCollectionProgress(0, carIds.length, '准备中...', carIds.length);
      ...
      for (let i = 0; i < carIds.length; i++) {
        ...
        const totalCars = startIndex + carIds.length;
        const remainingCars = carIds.length - i - 1;
        
        // 显示详细的采集进度
        logger.separator();
        logger.carCollectionProgress(absoluteIndex + 1, totalCars, carName, remainingCars);
        
        // 显示实时统计
        const totalProcessed = this.stats.successCount + this.stats.failCount;
        const estimatedTimePerCar = totalProcessed > 0 ? (Date.now() - this.stats.startTime) / totalProcessed : 30000; // 默认30秒每车型
        const estimatedRemaining = remainingCars * estimatedTimePerCar;
        ...
        try {
          // 为每个车型创建新的浏览器实例，添加连接检查
          try {
            browser = await this.browserManager.createBrowser();
            
            // 验证浏览器连接状态
            const recoveredBrowser = await this.dataCollector.recoverBrowserConnection(browser, `车型${carName}`);
            if (!recoveredBrowser) {
              throw new Error('浏览器连接验证失败，需要重新创建');
            }
            
          } catch (browserError) {
            logger.warning(`浏览器创建失败: ${browserError.message}，尝试重新创建...`);
            if (browser) {
              try {
                await this.browserManager.closeBrowser(browser);
              } catch (e) { /* 忽略关闭错误 */ }
          ...
          if (carData) {
            ...
            logger.success(`车型 ${carName} 采集成功 - ${carData.configs?.length || 0} 个配置`);
          } else {
            this.stats.failCount++;
            logger.error(`车型 ${carName} 采集失败 - 无有效数据`);
          }
          ...
          logger.error(`车型 ${carName} 处理错误: ${error.message.substring(0, 100)}`);
          this.stats.failCount++;
      ...
      const totalProcessed = this.stats.successCount + this.stats.failCount;
      const successRate = totalProcessed > 0 ? Math.round(this.stats.successCount/totalProcessed*100) : 0;
      
      // 显示最终采集完成状态
      logger.title(`${this.brandName} 品牌采集完成`);
      logger.carCollectionProgress(totalProcessed, totalProcessed, '全部完成', 0);
      logger.liveCollectionStatus({
        successCount: this.stats.successCount,
        failCount: this.stats.failCount,
        totalProcessed: totalProcessed,
      ...
      console.log(`📊 成功率: ${successRate}% (${this.stats.successCount}/${totalProcessed})`);
