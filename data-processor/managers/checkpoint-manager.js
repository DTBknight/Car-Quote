...
class CheckpointManager {
  ...
  createCarIdTracking(carIds, carNames) {
    ...
    carIds.forEach((carId, index) => {
      carIdStatus[carId] = {
        status: 'pending',
        carName: safeCarNames[index] || `车型${carId}`,
        attempts: 0,
        lastError: null,
        completedAt: null
      };
    ...
    return {
      ...
      carIdStatus: carIdStatus, // 新增详细状态跟踪  
      summary: summary,
      completedCarIds: [],
      pendingCarIds: [...carIds],
      failedCarIds: [],
      processedCount: 0,
      remainingCarIds: [...carIds] // 专门用于断点恢复的剩余ID列表
    };
  }
  ...
  updateCarIdStatus(carIdTracking, carId, status, error = null) {
    ...
    if (carIdTracking.carIdStatus[carId]) {
      carIdTracking.carIdStatus[carId].status = status;
      carIdTracking.carIdStatus[carId].attempts++;
      if (error) {
        carIdTracking.carIdStatus[carId].lastError = error;
      }
      if (status === 'completed') {
        carIdTracking.carIdStatus[carId].completedAt = new Date().toISOString();
      }
    }
    
    // 更新摘要统计
    const statusCounts = { pending: 0, completed: 0, failed: 0, inProgress: 0 };
    Object.values(carIdTracking.carIdStatus).forEach(car => {
      statusCounts[car.status] = (statusCounts[car.status] || 0) + 1;
    });
    
    carIdTracking.summary = {
      ...statusCounts,
      total: carIdTracking.totalCarIds
    };
