...
const { getSmartUserAgent, getRandomViewport, optimizePageLoad } = require('../utils/anti-detection');
...
class BrowserManager {
  ...
  async createPage(browser) {
    ...
    try {
      // 设置页面超时
      page.setDefaultTimeout(config.crawler.timeout || 300000); // 增加默认超时到5分钟
      page.setDefaultNavigationTimeout(config.crawler.timeout || 300000); // 增加导航超时到5分钟
